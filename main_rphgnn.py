...
if running_leaderboard_mag:
    ...
    num_views = 3
    cl_rate = 0.6
    model_save_dir = "/home/huangzhenghao2/RpHGNN/curri_model"
...
# else:
#       print("nrl cache path doesn't exist!")
# X = nrl_embedding_dict['paper'][:2000]
# output_activation_func = torch.nn.Softmax(dim=-1)
# X = output_activation_func(torch.Tensor(X))
...
# colors = plt.cm.Set1(np.linspace(0, 1, 349))
# x_min, x_max = X_tsne.min(0), X_tsne.max(0)
# X_norm = (X_tsne - x_min) / (x_max - x_min)  # 归一化
# plt.figure(figsize=(8, 8))
# for i in range(X_norm.shape[0]):
#       #if(y[i]==246):
#       #plt.text(X_norm[i, 0], X_norm[i, 1], str(y[i]), color=plt.cm.Set1(y[i]),fontdict={'weight': 'bold', 'size': 7})
#       plt.text(X_norm[i, 0], X_norm[i, 1], str(y[i]), color=colors[y[i]],fontdict={'weight': 'bold', 'size': 4})
# plt.xticks([])
# plt.yticks([])
